...
dwc_check_longitude_latitude <- function(longitude,
  ...
  if(base::missing(longitude)){
    base::stop(
      '"longitude" is missing, please provide the longitude', call. = FALSE)
  }

  if(base::missing(latitude)){
    base::stop(
      '"latitude" is missing, please provide the latitude', call. = FALSE)
  }
  ...
  if(verbatimCoordinateSystem == "degrees decimal minutes" &&
     (base::any(!stringr::str_detect(longitude,
                                     "\\d+\\s\\d+\\.\\d+\\s[EW]")) ||
      base::any(!stringr::str_detect(latitude,
                                     "\\d+\\s\\d+\\.\\d+\\s[NS]")) ) ) {
    base::stop(
